-- [[ نظام إشعار التحديث لمرة واحدة فقط ]] --

local HttpService = game:GetService("HttpService")
local player = game.Players.LocalPlayer

-- رقم التحديث (غير هذا الرقم كل ما تنزل تحديث جديد عشان تطلع الرسالة مرة ثانية للكل)
local UpdateVersion = "1.2" 

-- اسم الملف اللي بيحفظ فيه السكربت إن المستخدم شاف التحديث
local fileName = "SaadHub_UpdateCheck.txt"

-- وظيفة إرسال الإشعار بشعار روبلوكس
local function SendUpdateNotify()
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "🚀 SAADHUB UPDATED!";
        Text = "تم التحديث للنسخة " .. UpdateVersion .. "\nتم إضافة ميزات TVON والحماية";
        Icon = "rbxassetid://18835824901"; -- شعار السكربت (أو حط آيدي شعار روبلوكس)
        Duration = 8;
    })
end

-- فحص هل المستخدم شاف هذا التحديث من قبل؟
local function checkUpdate()
    local success, content = pcall(function()
        return readfile(fileName)
    end)

    -- إذا النسخة المحفوظة تختلف عن نسخة السكربت الحالية، أرسل الإشعار واحفظ النسخة الجديدة
    if not success or content ~= UpdateVersion then
        SendUpdateNotify()
        writefile(fileName, UpdateVersion)
    end
end
